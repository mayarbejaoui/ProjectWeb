<?php
	...
	class AvisC {
		...
		function ajouteravis($Avis,$id_rec,$rating){
			$sql="INSERT INTO avis (id_perso, message,id_rec, rating) 
			VALUES ( :id_perso, :message, :id_rec, :rating)";
			$db = config::getConnexion();
			try{
				$query = $db->prepare($sql);
				$query->execute([
		...
		function modifieravis($Avis, $id_avis,$rate){
